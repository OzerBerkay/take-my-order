...
import com.berkay.payment.service.domain.dto.message.PermissionPayload;
import com.berkay.payment.service.domain.ports.input.message.listener.permission.PermissionMessageListener;
...
public class IdentityPermissionKafkaListener implements KafkaConsumer<PermissionEventAvroModel> {
    ...
    @KafkaListener(id = "${kafka-consumer-config.permission-consumer-group-id}",
            topics = "${payment-service.permission-topic-name}")
